  ...
  connect.query("SELECT * FROM tamu WHERE kode = ? AND id_pernikahan = ?", [data.kode, data.id_pernikahan], (error, result) => {
    if (result.length > 0) {
      connect.query("SELECT * FROM tamu WHERE kode = ? AND id_pernikahan = ? AND rsvp = 'Belum Dikonfirmasi'",[data.kode, data.id_pernikahan],(error2,result2) => {
        if(result2.length > 0) {
          connect.query(
              "SELECT tamu.kode, detail_sesi.id_sesi, detail_sesi.id_tamu, sesi.nama_sesi, sesi.tanggal, sesi.waktu_mulai, sesi.waktu_selesai, sesi.alamat FROM tamu JOIN detail_sesi ON tamu.id = detail_sesi.id_tamu JOIN sesi ON sesi.id = detail_sesi.id_sesi WHERE tamu.kode = ?",
              [data.kode],
              (error1, result1) => {
      ...
      if (hasil.length <= 0) return res.json({ code: 0, message: "Kode tamu ini tidak valid!" });
        ...
        if (data.status === "Check In") {
          connect.query("SELECT * FROM tamu WHERE kode = ?", [data.kode], (error, result) => {
            if (result[0].kehadiran === "Check In") return res.json({ code: 0, message: "Tamu ini sudah melakukan Check In!" });
            else {
              connect.query("UPDATE tamu SET kehadiran = ?, waktu_checkin = CURTIME() WHERE kode = ? AND id_pernikahan = ?", [data.status, data.kode, data.id_pernikahan], (error1, result1) => {
                if (!error1) return res.json({ code: 1, message: "Berhasil menambahkan tamu ke buku tamu!" });
          ...
          connect.query("SELECT * FROM tamu WHERE kode = ?", [data.kode], (error, result) => {
            if (result[0].kehadiran === "Check Out") return res.json({ code: 0, message: "Tamu ini sudah melakukan Check Out!" });
            else if (result[0].kehadiran === "Belum Dikonfirmasi") return res.json({ code: 0, message: "Tamu ini belum melakukan Check In!" });
            else {
              connect.query("UPDATE tamu SET kehadiran = ?, waktu_checkout = CURTIME() WHERE kode = ? AND id_pernikahan = ?", [data.status, data.kode, data.id_pernikahan], (error1, result1) => {
                if (!error1) return res.json({ code: 1, message: "Berhasil memperbarui buku tamu!" });
                else return res.json({ code: 0, message: "Terjadi kesalahan!" });
              });
            }
          });
